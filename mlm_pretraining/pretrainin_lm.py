...
Original file is located at
    https://colab.research.google.com/drive/1VjqIQC9WTQ9iyFmioRaFZ4ZCeZ6QbeYb
"""
...
'''
def line_by_line_store(eval=True):
    data = pd.read_csv(f"{BASE_PATH}/Dataset/reddit_all_patient_updated_.csv")

    with open(f"{BASE_PATH}/Dataset/allpatient_train.txt", "w+") as f:
        if eval:
            f.writelines([f"{l}\n" for l in list(data['text'])[:-1000]])
        else:
            f.writelines([f"{l}\n" for l in list(data['text'])])
    if eval:
        with open(f"{BASE_PATH}/data/allpatient_eval.txt", "w+") as f:
            f.writelines([f"{l}\n" for l in list(data['text'])[-1000:]])
    
    del data

line_by_line_store(eval=False)
'''
