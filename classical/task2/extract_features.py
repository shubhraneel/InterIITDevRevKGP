...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-g", "--w2v_path", type=str, required=True, help="Path to word vector file"
    )
    parser.add_argument(
        "-v",
        "--version",
    ...
    parser.add_argument(
        "-c", "--cpu", action="store_true", help="Use CPU instead of GPU."
    )
    parser.add_argument(
        "-b", "--batch-size", type=int, default=64, help="Batch size (default: 64)"
    )
    parser.add_argument(
        "files", nargs="+", help="List of files to extract sentence embeddings"
    )
    ...
    for fpath in args.files:
        ...
        # Get embeddings
        embs = model.encode(
            sents, tokenize=args.tokenize, verbose=True, bsize=args.batch_size
        )
