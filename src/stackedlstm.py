...
class LSTM(pl.LightningModule):
    def __init__(self, vocab_size, config, train_dataloader = None, validation_dataloader = None, test_dataloader = None):
        ...
        self.lstm1 = nn.LSTM(config.model.params.input_size, self.hidden_size, self.num_layers, batch_first=True)
        self.lstm2 = nn.LSTM(config.model.params.input_size, self.hidden_size, self.num_layers, batch_first=True)
        self.lstm3 = nn.LSTM(config.model.params.input_size, self.hidden_size, self.num_layers, batch_first=True)
...
class StackedLSTMModel(Base_Model):
